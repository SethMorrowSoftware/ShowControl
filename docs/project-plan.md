...
> **This is the original strategy & implementation plan**, preserved for its
> market analysis, risk register, and roadmap. It predates the code and is **not**
> the as-built reference — some of its API sketches (e.g. `uint64_t` handles) were
> superseded during implementation. For what actually shipped, start with the
> [README](../README.md) and the [`docs/`](.) set (architecture, building,
> getting-started, api-reference). The forward-looking sections here — target
> users (§2), the showcase demos (§11), and the roadmap (§16) — remain the
